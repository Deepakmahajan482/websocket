  ...
  const[password,setPassword]=useState("");
    return (
      <div className='bg-gradient-to-b from-black to-purple-950  text-black h-screen grid place-items-center'>
        ...
        <p className='text-3xl font-extrabold'>Signin to  WeChat  </p>
          
          <label htmlFor='mail' className='text-xl m-2'>Email
            <br />
        <input type="text"  className='border border-gray-400 p-2 text-white pl-4 w-full mt-2 rounded-full' id='mail' placeholder='Enter your Email' value={mail} onChange={(e)=>{setMail(e.target.value)}} required/>
        ...
        <button className='cursor-pointer text-xl p-4 m-5 text-black rounded-4xl   mt-4 bg-purple-300 text-500' >Sign in </button>
        <span>Don't have the account ?<a href='/signup' className='text-blue-500 cursor-pointer' onClick={()=>{}}> Click here</a></span>
      </div>
  </form>
      </div>
    )
